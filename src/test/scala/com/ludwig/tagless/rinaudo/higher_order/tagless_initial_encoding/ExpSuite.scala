...
import com.ludwig.tagless.rinaudo.higher_order.tagless_initial_encoding.Exp.*
...
class ExpSuite extends munit.FunSuite {
  ...
  test("evaluate eq expression => false") {
    val expr = Eq(Lit(1), Lit(-4))
    assertEquals(eval(expr), false)
  }

  test("evaluate eq expression => true") {
    val expr = Eq(Lit(-4), Lit(-4))
    assertEquals(eval(expr), true)
  }
}

  // Following methods do not compile
